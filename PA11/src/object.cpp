...
void Object::Update(unsigned int dt, int index, Physics* world)
{
  btTransform trans;
  btScalar m[16];
...
void Object::Update(glm::mat4 parent, float angle)
{
    glm::vec3 v = glm::vec3(5*sin(2*angle), 0.0, 5*cos(2*angle)) + glm::vec3(parent[3]);
...
void Object::translateModel(int objectIndex,btVector3 newM, Physics* world, float charAngle)
{
    btTransform trans;
...
btCollisionShape *tempShape;
    ...
    world->bodies[objectIndex]->getMotionState()->getWorldTransform(original);
    ...
    btScalar m[16];
    trans.setOrigin(movePos);
    trans.setRotation(oldangle);

    
    world->bodies[objectIndex]->setWorldTransform(trans);
    world->bodies[objectIndex]->getMotionState()->setWorldTransform(trans);
  tempShape = world->bodies[0]->getCollisionShape();
  world->bodies[0]->setCollisionShape(  tempShape  );
    trans.getOpenGLMatrix(m);

    model = glm::make_mat4(m);
    model = glm::translate(model, glm::vec3(newM.x(), newM.y(), newM.z()));



}

/*void Object::translateModel(int objectIndex,int index2, Physics* world, float charAngle)
{
    btTransform trans;
    btTransform original;
btCollisionShape *tempShape;
    ...
    btQuaternion childAngle;
    btVector3 childPos;

    world->bodies[index2]->getMotionState()->getWorldTransform(original);
    childAngle = original.getRotation();
    childPos =  original.getOrigin();
   ...
   // movePos = (oldPos + newM);
    ...
    btScalar m[16];
    trans.setOrigin(childPos);
    trans.setRotation(childAngle);
...
void Object::rotateModel(int objectIndex,btQuaternion newM, Physics* world, float angle)
{
    btTransform trans;
    btTransform original;
    btVector3 oldPos;
    btQuaternion oldangle;
    btQuaternion moveAngle;
    ...
    moveAngle = oldangle * newM;

    btScalar m[16];
    trans.setOrigin(oldPos);
    trans.setRotation(moveAngle);
...
bool Object::toOriginalPostion()
{
   model = originModel;
   return (model == originModel);
}
